...
contract LiquidityManagementArbitrum  is IUniswapV3SwapCallback {
  ...
  function  uniswapV3SwapCallback  (
        ...
        require(msg.sender == poolAddress_weth_usdc_500 || msg.sender == poolAddress_weth_usdt_500 ||
        	msg.sender == poolAddress_weth_usdc_3000 || msg.sender == poolAddress_weth_usdt_3000, "bdsend");
...
// given an output amount of an asset and pair reserves, returns a required input amount of the other asset
   function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
	...
	TransferHelper.safeTransferFrom(
		    tokenToTransfer, msg.sender, poolAddress_sushi, amountIn
	);
	
	if(side)
    		pairV2.swap(amountOut, uint(0), msg.sender, new bytes(0));      
    	else
    		pairV2.swap(uint(0), amountOut, msg.sender, new bytes(0));        	
    }
...
function arb(uint16 arbNum, int128 amount0)  public {
       ...
       if(pool0==0)      ETH_USDC_Swap_500(amount0, isbuy);    	
       else if(pool0==1) ETH_USDC_Swap_3000(amount0, isbuy);    	
       else if(pool0==2) ETH_USDT_Swap_500(amount0, isbuy);    	
       else if(pool0==3) ETH_USDT_Swap_3000(amount0, isbuy);    	
       
       if(pool1==0)      ETH_USDC_Swap_500(amount0, !isbuy);    	
       else if(pool1==1) ETH_USDC_Swap_3000(amount0, !isbuy);    	
       else if(pool1==2) ETH_USDT_Swap_500(amount0, !isbuy);    	
       else if(pool1==3) ETH_USDT_Swap_3000(amount0, !isbuy);    	     
 }      
...
function arb2(uint32 arbNum, int128 amount0, int128 amount1)  public {
       ...
       if(pool0==0)      ETH_USDC_Swap_500(amount0, isbuy0);    	
       else if(pool0==1) ETH_USDC_Swap_3000(amount0, isbuy0);    	
       else if(pool0==2) ETH_USDT_Swap_500(amount0, isbuy0);    	
       else if(pool0==3) ETH_USDT_Swap_3000(amount0, isbuy0);    	
       
       if(pool1==0)      ETH_USDC_Swap_500(amount1, isbuy1);    	
       else if(pool1==1) ETH_USDC_Swap_3000(amount1, isbuy1);    	
       else if(pool1==2) ETH_USDT_Swap_500(amount1, isbuy1);    	
       else if(pool1==3) ETH_USDT_Swap_3000(amount1, isbuy1);    	     
 }      
